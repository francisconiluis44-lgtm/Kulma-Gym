...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (data.user?.email?.endsWith('@kulmagym.app')) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl px-8 py-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-white/70 mb-1.5 font-body">
                Email
              </label>
              <input
                type="email"
            ...
            {error && (
              <p className="text-red-400 text-sm font-body bg-red-400/10 px-3 py-2 rounded-lg">
                {error}
              </p>
            )}
        ...
        <p className="mt-6 text-center">
          <Link
            href="/login"
            className="text-xs text-white/30 hover:text-white/50 transition-colors font-body"
          >
